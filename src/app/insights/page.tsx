...
export default function InsightsPage() {
  ...
  const sovData = (() => {
    ...
    history.forEach(scan => {
      ['gpt', 'claude', 'gemini', 'perplexity'].forEach(m => {
        ...
        // Sécurité sur les noms des concurrents
        ...
        // Sécurité sur la marque principale
  ...
  const sentimentStats = (() => {
    ...
    ['gpt', 'claude', 'gemini', 'perplexity'].forEach(m => {
      ...
      // Sécurité : forcer en string et formater proprement
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-6 text-left">
            ...
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Share of Voice (Recent History)</h3>
          </div>
                ...
                <div key={item.name} className="flex flex-col gap-1 text-left">
                  ...
                  <div className="h-1 w-full bg-slate-50 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${item.percentage}%`, backgroundColor: getBrandColor(item.name) }}
                    ></div>
                  </div>
                </div>
              )) : <p className="text-xs text-slate-300 italic">Awaiting data analysis...</p>}
            </div>
          </div>
        </div>
